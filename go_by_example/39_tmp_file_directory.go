...
func main() {
	...
	*/
	...
	/*
		打印临时文件的名称。
		文件名以 ioutil.TempFile 函数的第二个参数作为前缀， 剩余的部分会自动生成，
		以确保并发调用时，生成不重复的文件名。 在类 Unix 操作系统下，临时目录一般是 /tmp。
	*/
	...
	// ioutil.TempDir 的参数与 TempFile 相同， 但是它返回的是一个目录名 ，而不是一个打开的文件。
	dname, err := ioutil.TempDir("", "sampledir")
	fmt.Println("temp dir name:", dname)

	defer os.RemoveAll(dname)

	// 可以通过拼接临时目录和临时文件合成完整的临时文件路径，并写入数据。
